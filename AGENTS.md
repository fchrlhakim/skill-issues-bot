...
- The bot calls skill-issues-saas in exactly one place: an optional, read-only `GET` of the SaaS admin finance overview, used by `/revenue` (`modules/discordbot/bot.go`). Keep it read-only and optional: with no credential configured, `/revenue` must report `SaaS revenue: unavailable` rather than fail. Authenticate with `SAAS_REFRESH_TOKEN`, which the bot rotates against the SaaS (`modules/discordbot/saas_token.go`) and persists under `storage/`; never present an already-rotated refresh token, because the SaaS revokes the whole token family on reuse. Never share a database with the SaaS. Do not add a client to skill-issues-proxy, and do not widen this into writes, shared secrets, or a shared store.
